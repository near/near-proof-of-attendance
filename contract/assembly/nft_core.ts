...
import { JsonToken, Token } from "./token";

import { Gas, AccountId, Balance, TokenId } from "./types";
...
export function internal_nft_transfer(receiver_id: AccountId, token_id: TokenId, approval_id: u64, memo: string): void {
  ...
  const previous_token: Token = internal_transfer(
    sender_id,
    receiver_id,
    token_id,
    approval_id,
...
export function internal_nft_tokens_for_owner(
  account_id: string
): Token[] | null {
  const tokenIds = TokensPerOwner.get(account_id);
  const userTokens = new Array<Token>();

  if (tokenIds) {
    for (let i = 0; i < tokenIds.values().length; i++) {
      const metadata = TokensById.get(tokenIds.values()[i], null);
      if (metadata) {
